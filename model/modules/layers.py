...
from model.modules.enum_type import FeatureType
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, queries,keys , attention_mask):
        attention_output = self.multi_head_attention(queries, keys,attention_mask)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, queries,keys, attention_mask, output_all_encoded_layers=True):
        ...
        for layer_module in self.layer:
            queries = layer_module(queries,keys, attention_mask)
...
class StructureAwareMultiHeadAttention(nn.Module):
    ...
    """
    def __init__(self, n_heads, hidden_size, max_key_length,hidden_dropout_prob, attn_dropout_prob, layer_norm_eps):
        super(StructureAwareMultiHeadAttention, self).__init__()
        if hidden_size % n_heads != 0:
            raise ValueError(
                "The hidden size (%d) is not a multiple of the number of attention "
                "heads (%d)" % (hidden_size, n_heads))
        ...
        self.structure_bias = nn.Parameter(torch.Tensor(self.max_key_length))
...
class StructureAwareTransformerLayer(nn.Module):
    ...
    """
    def __init__(self, n_heads, hidden_size,max_key_length, intermediate_size,
                 hidden_dropout_prob, attn_dropout_prob, hidden_act, layer_norm_eps):
        super(StructureAwareTransformerLayer, self).__init__()
        self.multi_head_attention = StructureAwareMultiHeadAttention(
                                    n_heads=n_heads,
    ...
    def forward(self, queries,keys ,queries_graph_emb, keys_graph_emb, attention_mask):
        attention_output = self.multi_head_attention(queries, keys,queries_graph_emb, keys_graph_emb,attention_mask)
        feedforward_output = self.feed_forward(attention_output)
        return feedforward_output
...
class StructureAwareTransformerEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        layer = StructureAwareTransformerLayer(n_heads=n_heads,
                                               hidden_size=hidden_size,
                                               max_key_length=max_key_length,
    ...
    def forward(self, queries,keys, queries_graph_emb,keys_graph_emb,attention_mask, output_all_encoded_layers=True):
        """
        Args:
            hidden_states (torch.Tensor): the input of the TrandformerEncoder
            attention_mask (torch.Tensor): the attention mask for the input hidden_states
        ...
        all_encoder_layers = []
        for layer_module in self.layer:
            queries = layer_module(queries,keys, queries_graph_emb, keys_graph_emb,attention_mask)
            if output_all_encoded_layers:
                all_encoder_layers.append(queries)
        if not output_all_encoded_layers:
            all_encoder_layers.append(queries)
