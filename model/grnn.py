...
class GRNN(SequentialRecommender):
    ...
    def __init__(self, config, item_num):
        ...
        # self.output_dropout = nn.Dropout(self.dropout_probs[1])
        # position embedding
        ...
        self.tmp2 = nn.Parameter(torch.Tensor(self.max_seq_length,self.max_seq_length))
    ...
    def forward(self, item_seq, adj_in,adj_out,item_seq_len):
        ...
        """
        # READOUT 0
        # mask = item_seq.gt(0).unsqueeze(2).expand_as(gru_output)
        #
        ...
        #
        # att_val_inputs = (trans_item_embedding+trans_last_item_embedding+self.r)*mask
        #
        ...
        #
        # x_global = torch.sum(att_val* gru_output,1)
        # c_t = torch.cat((x_last,x_global),1)
        # c_t = self.output_dropout(c_t)
        #
        # seq_output = self.output_layer(c_t)
        ...
        # mask = item_seq.gt(0).unsqueeze(2).expand_as(gru_output) # batch_size, max_len, num_units
        #
        # x_last = self.gather_indexes(gru_output, item_seq_len - 1)
        #
        # pos_gru_output = self.tmp1(torch.cat((gru_output, position_embedding), 2))
        #
        # att_val = self.tmp2  #  max_len,max_len
        #
        # x_global = torch.matmul(att_val , pos_gru_output) * mask
        # x_global = torch.sum(x_global,1)
        #
        # c_t = torch.cat((x_last, x_global), 1)
        # c_t = self.output_dropout(c_t)
        #
        ...
        # pos_gru_output = self.tmp1(torch.cat((gru_output, position_embedding),2))
        #
        # trans_item_embedding = self.W1(gru_output)
        # trans_last_item_embedding = self.W2(x_last).view(-1,1,self.embedding_size)
        #
        # att_val_inputs = (trans_item_embedding+trans_last_item_embedding)*mask
        #
        # att_val = self.q(att_val_inputs).view(-1,self.max_seq_length)
        # att_val = att_val.view(-1,self.max_seq_length,1) # batch_size, max_len,1
        #
        ...
        # att_val = self.q(att_val_inputs).view(-1, self.max_seq_length)
        # att_val = att_val.view(-1, self.max_seq_length, 1)  # batch_size, max_len,1
        # att_val = self.output_dropout(att_val)
        #
        # x_global = torch.sum(att_val * gru_output, 1)
        # c_t = torch.cat((x_last, x_global), 1)
        #
        ...
        # trans_item_embedding = self.W1(gru_output)
        # trans_last_item_embedding = self.W2(x_last).view(-1, 1, self.embedding_size)
        # att_val_inputs = torch.sigmoid(trans_item_embedding + trans_last_item_embedding) * mask
        #
        # att_val = self.q(att_val_inputs).view(-1, self.max_seq_length)
        # att_val = att_val.view(-1, self.max_seq_length, 1)  # batch_size, max_len,1
        # att_val = self.output_dropout(att_val)
        ...
        # att_val = att_val.view(-1, self.max_seq_length, 1)  # batch_size, max_len,1
        #
        # x_global = torch.sum(att_val * pos_gru_output, 1)
        # c_t = torch.cat((x_last, x_global), 1)
        # c_t = self.output_dropout(c_t)
        #
        # seq_output = self.output_layer(c_t)
