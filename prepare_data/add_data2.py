...
#             # u_lst = np.where(u_input == u_input[i])[0]
#             u_lst = item2idx[u_input[i]]
#             for j in np.arange(i+1, len(u_input), 1): #TODO 都不加自环了
#
#                 tuple = (u_input[i], u_input[j])
#                 if tuple in processed:
#                     continue
...
#             # u_lst = np.where(u_input == u_input[i])[0]
#             u_lst = item2idx[u_input[i]]
#             for j in np.arange(0,i , 1): # TODO 都不加自环了
#                 tuple = (u_input[i],u_input[j])
#                 if tuple in processed:
#                     continue
#                 processed[tuple] = True
...
if __name__ == "__main__":
    ...
    for dir in ['home_time_item_based_unidirection']:
        dir_path = root+dir+'/'
        print(dir_path)

        for filename in ['train_data.txt','test_data.txt','dev_data.txt']:
            new_filename = filename[:-4]+'_new2'+'.txt'
            i = 0
